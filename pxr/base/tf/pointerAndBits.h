...
#ifdef ARCH_BITS_32
// If we are on a 32 bit architecture, we just store 32bits and a pointer with
// 32 bits Taken from:
// https://github.com/autodesk-forks/USD/blob/adsk/collab/port-python-wasm/pxr/base/tf/pointerAndBits.h
...
public:
  ...
  constexpr uintptr_t GetNumBitsValues() const { return 32; }
  ...
  constexpr uintptr_t GetLiteral() const noexcept {
    return (static_cast<uint64_t>(_bits) << 32) |
           reinterpret_cast<uint64_t>(_ptr);
  }
...
private:
  ...
  constexpr T *_GetPtr() const noexcept { return _ptr; }

  // Set the held pointer value.
  void _SetPtr(T *p) noexcept { _ptr = p; }

  // Retrieve the held bits value.
  constexpr uint32_t _GetBits() const noexcept { return _bits; }

  // Set the held bits value.
  void _SetBits(uint32_t bits) noexcept { _bits = bits; }
