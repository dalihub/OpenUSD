...
static std::atomic<int64_t> Arch_IntervalTimerTickOverhead{
    UninitializedState<int64_t>};
...
static int64_t Arch_ComputeTickQuantum() {
  ...
  for (int trial = 0; trial != NumTrials; ++trial) {
    ...
    currMin = std::min(currMin, *std::min_element(std::begin(times),
                                                  std::prev(std::end(times))));
  }
...
uint64_t Arch_MeasureExecutionTime(uint64_t maxTicks, bool *reachedConsensus,
                                   void const *m,
